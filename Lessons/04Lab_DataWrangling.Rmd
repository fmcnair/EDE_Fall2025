...
filter(NTL.phys.data, year4 %in% 1999)
# Exercise 2: 
...
> Answer: don't know if the data is in order or not 
...
NTL.phys.data.processed_3 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999) & daynum %in% c(182:212))
  
dim(NTL.phys.data.processed_3)
...
names(NTL.phys.data)
combined <- pivot_longer(NTL.phys.data, cols = irradianceWater:irradianceDeck, names_to='Irradiance_Type', values_to = "Irradiance_Value")
